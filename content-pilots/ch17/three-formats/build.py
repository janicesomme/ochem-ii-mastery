...
content='<header><p class="eyebrow">NO FEAR OCHEM · CHAPTER 17 · V1</p><h1>Same reaction.<br>Three ways to ask.</h1><p>Find the pattern. Make the move. Use it whichever part of the reaction is missing.</p></header><section><h2>1 · Learn these two pairs</h2><p><strong>Keep the condition with the fact:</strong> these patterns apply when NaBH₄ reduces an aldehyde or ketone. Other reagents can make those same starting groups react differently.</p>'
...
(P/'three-question-formats-v1.html').write_text('<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Same reaction, three question formats — v1</title><style>'+css+'</style><main>'+content+'</main></html>')
